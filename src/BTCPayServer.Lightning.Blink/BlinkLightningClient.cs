...
namespace BTCPayServer.Lightning.Blink
{
    public class BlinkLightningClient : ILightningClient
    {
        ...
        private readonly BlinkClient _blinkClient;

        public BlinkLightningClient(Uri address, string password, Network network, HttpClient httpClient = null) :
            this(address, null, password, network, httpClient)
        {
        }

        public BlinkLightningClient(Uri address, string username, string password, Network network, HttpClient httpClient = null)
        {
            if (address == null)
            ...
            _password = password;
            _network = network;
            _blinkClient = new BlinkClient(address, username, password, network, httpClient);
        }
        ...
        public async Task<LightningInvoice> GetInvoice(string invoiceId, CancellationToken cancellation = default)
        {
            try
            {
                var result = await _blinkClient.GetInvoice(invoiceId, cancellation);
                return await ToLightningInvoice(result, cancellation);
            }
            catch (BlinkClient.BlinkApiException ex) when (ex.Error.Error == "Not found" || ex.Error.Error.Contains("Invalid hexadecimal", StringComparison.OrdinalIgnoreCase))
            {
                return null;
            }
        }
        ...
        public async Task<LightningInvoice[]> ListInvoices(ListInvoicesParams request, CancellationToken cancellation = default)
        {
            ...
            var invoices = request is { PendingOnly: true }
                ? await _blinkClient.ListPendingInvoices(from, null, cancellation)
                : await _blinkClient.ListInvoices(from, null, cancellation);
        ...
        private async Task<LightningInvoice> ToLightningInvoice(InvoiceResponse invoice, CancellationToken cancellation)
        {
            ...
            try
            {
                info = await _blinkClient.GetReceivedInfo(invoiceId, null, cancellation);
            }
            catch (BlinkClient.BlinkApiException)
            {
            }
        ...
        {
            var result = await _blinkClient.GetSentInfo(paymentHash, null, cancellation);
        ...
        {
            var result = await _blinkClient.CreateInvoice(
                description,
                amount.MilliSatoshi,
        ...
        {
            var info = await _blinkClient.GetInfo(cancellation);
        ...
        {
            var globalBalance = _blinkClient.GlobalBalance(cancellation);
            var usableBalances = _blinkClient.UsableBalances(cancellation);
            ...
            { 
                ...
                Reserved = null // Not supported by Blink
            };
        ...
        public async Task<PayResponse> Pay(string bolt11, PayInvoiceParams payParams, CancellationToken cancellation = default)
        {
            ...
            try
            {
                ...
                var uuid = await _blinkClient.PayInvoice(req, cts.Token);
                while (!cts.Token.IsCancellationRequested)
                {
                    var status = await _blinkClient.GetSentInfo(null, uuid, cts.Token);
                    if (!status.Any())
                    {
                        continue;
                    }
            ...
            catch (BlinkClient.BlinkApiException exception)
            {
        ...
        public async Task<PayResponse> Pay(PayInvoiceParams payParams, CancellationToken cancellation = default)
        {
            ...
            try
            {
                ...
                var uuid = await _blinkClient.SendToNode(req, cts.Token);
                while (!cts.Token.IsCancellationRequested)
                {
                    var status = await _blinkClient.GetSentInfo(null, uuid, cts.Token);
                    if (!status.Any())
            ...
            CancellationToken cancellation = default)
        {
            try
            {
                var result = await _blinkClient.Open(openChannelRequest.NodeInfo.NodeId,
                ...
                if (result.Contains("created channel", StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    var channel = await _blinkClient.Channel(channelId, cancellation);
        ...
        {
            return _blinkClient.GetNewAddress(cancellation);
        }
        ...
        public async Task<ConnectionResult> ConnectTo(NodeInfo nodeInfo, CancellationToken cancellation = default)
        {
            try
            {
                var result = await _blinkClient.Connect(nodeInfo.NodeId, nodeInfo.Host, nodeInfo.Port, cancellation);
        ...
        {
            var channels = await _blinkClient.Channels(null, cancellation);
        ...
        {
            var result= $"type=blink;server={_address}";
            if (_username is { })
